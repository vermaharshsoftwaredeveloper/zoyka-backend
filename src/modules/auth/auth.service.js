...
const sendOtpEmail = async ({ email, otp, purpose }) => {
  let actionLabel, subject, text;

  switch (purpose) {
    case "LOGIN":
      actionLabel = "login";
      subject = "Zoyka Login OTP";
    ...
    default:
      ...
      subject = "Zoyka Account Verification OTP";
...
export const resendOtpService = async ({ email, purpose }) => {
  ...
  if (purpose === "PASSWORD_RESET" && !user.isEmailVerified) {
  ...
  return {
    message: `OTP resent for ${purpose.toLowerCase().replace('_', ' ')} verification`,
  };
};
...
export const forgotPasswordService = async ({ email }) => {
  ...
  // Always use dynamic OTP for password reset (security-critical)
  const otp = generateOtp();
  const otpExpiry = getOtpExpiry();
