...
const processAvatar = async (avatar) => {
    ...
    try {
        return await uploadImage(avatar, "artisans", "artisan");
    } catch {
        return avatar;
    }
};
...
export const updateArtisanService = async (id, data) => {
    const outlet = await prisma.outlet.findUnique({
        where: { id },
        include: { owner: { select: { avatar: true } } },
    });
    ...
    if (outlet.ownerId) {
        ...
        if (avatarUrl) {
            ...
            if (outlet.owner?.avatar && outlet.owner.avatar !== avatarUrl) {
                await deleteImage(outlet.owner.avatar);
            }
            ownerUpdate.avatar = avatarUrl;
        }
