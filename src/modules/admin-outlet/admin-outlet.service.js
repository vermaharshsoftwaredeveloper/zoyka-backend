...
export const createAdminOutletService = async (payload) => {
  ...
  if (payload.image && typeof payload.image === "string") {
    try {
      payload.imageUrl = await uploadImage(payload.image, "outlets", "outlet");
...
export const updateAdminOutletService = async (outletId, payload) => {
  const existing = await getAdminOutletByIdService(outletId);

  // Process image on update (base64 or URL)
  if (payload.image && typeof payload.image === "string") {
    try {
      payload.imageUrl = await replaceImage(payload.image, existing.imageUrl, "outlets", "outlet");
      delete payload.image;
    } catch (err) {
      throw new ApiError(400, "Failed to process outlet image");
    }
